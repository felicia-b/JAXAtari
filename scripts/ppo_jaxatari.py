...
# c:\Users\felic\Documents\Studium\Bachelorarbeit\symbolic_options\src\symbolic_options\reward_functions\pong.pyfrom symbolic_options.utils.video_recorder import video_callback
...
def make_train(config):
    ...
    #     from jaxatari.wrappers import PixelObsWrapper, NormalizeObservationWrapper
    ...
    if config.get("ENV_NAME", None) == "Amidar":
        from jaxatari.wrappers import MultiRewardLogWrapper#, MultiRewardWrapper
        from jaxatari.wrappers import PixelObsWrapper, NormalizeObservationWrapper
        from jaxatari.games.jax_amidar import JaxAmidar, AmidarRenderer
        from jaxatari.games.amidar_reward_functions import env_reward

        reward_funcs = [env_reward, env_reward, env_reward] #use env_reward for all options

        sticky_actions = config.get("STICKY_ACTIONS", False)
        episodic_life = config.get("EPISODIC_LIFE", False)
        def create_env(train=False):
            env = JaxAmidar(reward_funcs=reward_funcs)
            if train:
                env = AtariWrapper(env, sticky_actions=sticky_actions, episodic_life=episodic_life, first_fire=False)
            else:
                env = AtariWrapper(env, sticky_actions=False, episodic_life=False, first_fire=False)
